<template>
    <div class="skills view-page">
        <div class="text">
            <h1 class="title">
                <div class="outline-text">Skills</div>
                <span class="hov">W</span>
                <span class="hov">h</span>
                <span class="hov">a</span>

                <span class="hov">t</span>
                <span class="space"></span>
                <span class="hov">I</span>
                <span class="space"></span>
                <span class="hov">D</span>
                <span class="hov">o</span>
            </h1>

            <p>
                The main area of my expertise is front end development (client
                side of the web).
            </p>

            <p>
                Build beautiful and interactive Web Application with HTML, CSS, and
                Javascript. I also have full stack development experience with Node.Js,
                MySQL, and Laravel.
            </p>

            <p>
                I also have offensive, defensive, forensic experience.
            </p>
            <p>
                Visit my
                <a
                    target="_blank"
                    href="https://www.linkedin.com/in/mauliafaqihuddin/"
                    class="link linkedln"
                    >Linkedln</a
                >
                profile for more details or just
                <span @click="menuBackgroundAnim(4)"
                    ><router-link to="/contact" class="link contact"
                        >contact me.</router-link
                    ></span
                >
            </p>
        </div>
        <div class="illus">
            <!-- <h1>My Tools</h1> -->
            <div class="front__end">
                <div
                    class="react stack"
                    @click="
                        showInfo('Laravel', 'laravel', 'A PHP framework')"
                >
                    <p>Laravel</p>

                    <span>
                        <i class="fab fa-laravel"></i>
                    </span>
                </div>
                <div
                    class="vue stack"
                    @click="showInfo('Vue.Js', 'vue', 'A Javascript framework')"
                >
                    <p>Vue.Js</p>

                    <span>
                        <i class="fab fa-vuejs"></i>
                    </span>
                </div>
                <div
                    class="node stack"
                    @click="
                        showInfo(
                            'Node Js',
                            'node',
                            'A Javascript runtime environment that executes JavaScript code outside a web browser. For example for Backend.'
                        )"
                >
                    <p>Node.Js</p>

                    <span>
                        <i class="fab fa-node"></i>
                    </span>
                </div>
                <div
                    class="js stack"
                    @click="
                        showInfo(
                            'Javascript',
                            'javascript',
                            'JavaScript is the Programming Language for the Web'
                        )
                    "
                >
                    <p>Javascript</p>

                    <span>
                        <i class="fab fa-js-square"></i>
                    </span>
                </div>
                <div
                    class="css stack"
                    @click="
                        showInfo(
                            'CSS',
                            'css',
                            'CSS is a tool for Create beautiful, responsive layout, and implementing the design into the Web App'
                        )
                    "
                >
                    <p>CSS</p>

                    <span>
                        <i class="fab fa-css3-alt"></i>
                    </span>
                </div>
                <div
                    class="html stack"
                    @click="
                        showInfo(
                            'HTML',
                            'html',
                            'HTML is a standard for describing a structure and presentation of information in a web.'
                        )
                    "
                >
                    <p>HTML</p>

                    <span>
                        <i class="fab fa-html5"></i>
                    </span>
                </div>
            </div>
            <div class="back__end">
                <div
                    class="mongo stack"
                    @click="
                        showInfo(
                            'Specialized Offensive Operations',
                            'offensive',
                            'Specialized Offensive Operations : Focused Areas & Techniques(Pen Testing & Cloud, Specialized Penetration Testing, Red Team, Purple Team'
                        )
                    "
                >
                    <p>Spec Offensive</p>

                    <span>
                        <i class="fas fa-database"></i>
                    </span>
                </div>
                <div
                    class="mongo stack"
                    @click="
                        showInfo(
                            'Offensive Operation',
                            'offensive',
                            'Offensive Operation : Vulnerability Analysis, Penetration Testing'
                        )
                    "
                >
                    <p>Offensive Op</p>

                    <span>
                        <i class="fas fa-lock"></i>
                    </span>
                </div>
            </div>
        </div>

        <div class="skills-info" ref="skillInfo">
            <div class="card">
                <div class="logo">
                    <img :src="require('../../src/assets/' + icon + '.svg')" />
                </div>
                <h3>{{ title }}</h3>
                <p>{{ content }}</p>
                <div class="close" @click="closeInfo">
                    <i class="fas fa-times"></i>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
import TransitionMixin from "../mixins/transition";
export default {
    mixins: [TransitionMixin],
    data() {
        return {
            title: "",
            icon: "vue",
            content: "",
        };
    },

    methods: {
        showInfo(title, icon, content) {
            this.title = title;
            this.icon = icon;
            this.content = content;
            let info = this.$refs.skillInfo;
            info.classList.toggle("show");
        },
        closeInfo() {
            let info = this.$refs.skillInfo;
            info.classList.toggle("show");
        },
    },
};
</script>

<style scoped src="../styles/skills.css"></style>
